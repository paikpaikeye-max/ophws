...
export default function AdminUsersPage() {
    ...
    const handleUpdateStatus = async (userId, newStatus) => {
        // 관리자 해제 시 최소 인원 체크
        if (newStatus === 'editor') {
            const currentAdmin = users.find(u => u.id === userId && u.status === 'admin');
            if (currentAdmin) {
                const adminCount = users.filter(u => u.status === 'admin').length;
                if (adminCount <= 1) {
                    alert('최소 1명의 관리자가 존재해야 합니다.');
                    return;
                }
            }
        }
    ...
    const handleDeleteUser = async (userId) => {
        ...
        if (!confirm(`정말로 '${user.name}' 사용자를 삭제(탈퇴) 하시겠습니까?\n프로필 데이터가 영구 삭제됩니다.`)) return
                                                ...
                                                <div className="text-xs text-slate-500">
                                                    {user.department === '기타' ? (
                                                        <div className="flex flex-col gap-0.5 mt-1">
                                                            <span className="text-blue-600 dark:text-blue-400">📝 {user.introduction || '자기소개 없음'}</span>
                                                            <span className="text-slate-400">📞 {user.contact || '-'}</span>
                                                        </div>
                                                    ) : (
                                                        user.employee_id || '-'
                                                    )}
