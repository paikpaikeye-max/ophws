...
function normalizeIolLensOverride(override = {}) {
    return {
        price: override.price || '',
        prosText: override.prosText || '',
        consText: override.consText || '',
        targetText: override.targetText || '',
    };
}

function normalizeIolSettings(iol) {
    const normalizedOverrides = {};

    if (iol?.lensOverrides && typeof iol.lensOverrides === 'object') {
        Object.entries(iol.lensOverrides).forEach(([lensId, override]) => {
            normalizedOverrides[lensId] = normalizeIolLensOverride(override);
        });
    }

    return {
        ...deepClone(DEFAULT_ASSISTANT_CONFIG.iol),
        showPrice: Boolean(iol?.showPrice),
        monochromePrint: Boolean(iol?.monochromePrint),
        lensOverrides: normalizedOverrides,
    };
}

function mergeIolSettings(baseIol, sourceIol, sections) {
    const base = normalizeIolSettings(baseIol);
    const source = normalizeIolSettings(sourceIol);
    const merged = normalizeIolSettings(base);
    const lensIds = new Set([...Object.keys(base.lensOverrides), ...Object.keys(source.lensOverrides)]);

    if (sections.includes('pricing')) {
        merged.showPrice = source.showPrice;
    }
    if (sections.includes('printStyle')) {
        merged.monochromePrint = source.monochromePrint;
    }

    lensIds.forEach((lensId) => {
        const baseOverride = normalizeIolLensOverride(base.lensOverrides[lensId]);
        const sourceOverride = normalizeIolLensOverride(source.lensOverrides[lensId]);

        merged.lensOverrides[lensId] = {
            ...baseOverride,
            price: sections.includes('pricing') ? sourceOverride.price : baseOverride.price,
            prosText: sections.includes('content') ? sourceOverride.prosText : baseOverride.prosText,
            consText: sections.includes('content') ? sourceOverride.consText : baseOverride.consText,
            targetText: sections.includes('content') ? sourceOverride.targetText : baseOverride.targetText,
        };
    });
...
export function normalizeAssistantConfig(config) {
    ...
    normalized.iol = normalizeIolSettings(config.iol);
...
function applyDeploySections(baseConfig, sourceConfig, deployMeta) {
    ...
    if (deployMeta?.featureId === 'iol') {
        if (sections.includes('visibility')) {
            updated.featureVisibility.iol = source.featureVisibility.iol;
        }
        if (sections.includes('content') || sections.includes('pricing')) {
            updated.iol = mergeIolSettings(updated.iol, source.iol, sections);
        }
    }
...
export function mergeConfig(globalConfig, userOverrides) {
    ...
    if (userOverrides.iol !== undefined) {
        merged.iol = normalizeIolSettings(userOverrides.iol);
    }
